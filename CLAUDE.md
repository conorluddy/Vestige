...
**Shipped:** V0 (M0–M5), V0.1, V0.2, V0.3, V0.4, V0.4.1, V0.5, V0.5.1, V0.5.2 (menu-bar controls — Rust verified; Swift authored, needs a macOS build), V0.5.3 (agent-driven session-log ingestion), V0.5.4 (autonomous session-log ingestion — daemon + CLI).
...
- V0.5.2 (**Menu-bar controls**) turned the read-only app into a control surface (epic #88 phase 2 + the EXP-1..7 experience layer #114–#120). **Rust side (fully tested):** additive `daemon.pause`/`daemon.resume` IPC + `paused_until` status field + scheduler tick-suppression (status tick never skipped, pause auto-expires); `vestige daemon pause --for/--until` / `resume` controllers; `vestige ui` launcher + a strictly-guarded init/daemon-install boot prompt (`commands::ui::decide_boot` — the agent-safety invariant, never fires in agent/CI/non-TTY/non-macOS; `--no-ui`/`--yes` flags). **Swift side (authored; needs a macOS build):** `DaemonSocket` JSON-RPC client, menu actions, pause affordance, persistent workspace `Window`, `SMAppService` Login Item, plus EXP-1 activity pulse, EXP-2 inbox badge, EXP-3 Liquid-Glass/centralised-status polish, EXP-4 app-local sparkline, EXP-5/6 memory peek (recent ticker + ⌘-search via a CLI shell-out bridge), EXP-7 ⌥⌘N quick capture (`vestige note add`). No new daemon write paths; `schema_version` stays 1. Spec: `docs/prd/vestige_v_0_5_2_menubar_controls_prd.md`; walkthrough: `docs/v0.5.2.md`.
