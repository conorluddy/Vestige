...
- **Manual recovery** is `cargo publish -p <crate>` per crate in topological order: `vestige-core → vestige-embed → vestige-store → vestige-config → vestige-engine → vestige-mcp → vestige-daemon → vestige`. Keep this list aligned with the workspace dep graph if a crate is added.
